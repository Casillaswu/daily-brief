...
### 7.24 每行容量第三次修正：signal 卡 EN 容量差一倍、§9 卡被當 col 量（2026-09-14 W37 線上實測）

**§7.23 的表只量了 lead / signal / col 三個 CN 數字就外推 EN，結果 EN signal 的門檻比實際低一倍；而 §9 的兩張卡從來沒被單獨量過。**

W37 推上 GitHub Pages 後用 Chrome 實測（viewport 1163px、桌面三欄），CN 與 EN 都是 **0 個三行標題**，但各有 4 個孤行：

| | 孤行 | 末行填充率 |
|---|---|---|
| CN | 4 個 | 訊號 04 / 訊號 05 / §8 NFL 各 37%（差 1 點）、**§9 巴西卡 9%** |
| EN | 4 個 | 訊號 02 / 03 / 04 / 05 分別 18% / 12% / 7% / 12% |

而 `check_headline_len.py` 對這 8 個全部回報合格。兩個根因：

**根因 1：`CAP_EN['signal']` 寫 25 當量（= 50 字元），實測是 65–70 字元。** 低了一倍，所以它把 71–83 字元的標題判成「兩行飽滿」，實際是兩行、末行只剩 7–18%。更糟的是它**反向誤導寫作**——W37 第一版 EN 訊號標題本來寫 102–133 字元（實測末行 45–61%、完全合格），是被這個錯門檻擋下來才砍到 71–83 字元，砍完才變孤行。**工具給錯的紅燈跟給錯的綠燈一樣有害。**

**根因 2：§9 的卡 class 是 `.card`，但它在 `.x-pulse` 兩欄格線裡、實寬 504px，不是 col 的 318px。** 檢查器用 col 的 21 當量去量，一個 37.5 當量的標題被算成「兩行、末行 78%」，實際是兩行、末行 9%。

**實測容量（此表取代 §7.23，為現行真值）**：

| 區塊 | 選擇器 | 卡寬 | CN 每行 | EN 每行 |
|---|---|---|---|---|
| lead h3 | `article.lead h3` | 1050px | 61 全角字 | 125 字元 |
| signal h4 | `.signal-card h4` | 504px | 34 全角字 | **65–70 字元**（≈ 34 當量） |
| col h4 | `.grid-auto .card h4` | 318px | 21 全角字 | 36 字元 |
| **x-pulse h4** | `.x-pulse .card h4` | **504px** | **34 全角字** | **≈ 34 當量**（與 signal 同） |

**據此的目標長度**（孤行門檻沿用 CN 38% / EN col 30%）：

| 區塊 | CN 單行 | CN 兩行飽滿 | EN 單行 | EN 兩行飽滿 |
|---|---|---|---|---|
| lead | ≤ 61 字 | — | ≤ 125 字元 | — |
| signal | ≤ 34 字 | 47–68 字 | ≤ 64 字元 | **91–132 字元** |
| col | ≤ 21 字 | 29–42 字 | ≤ 36 字元 | 50–72 字元 |
| x-pulse | ≤ 34 字 | 47–68 字 | ≤ 64 字元 | 91–132 字元 |

**`check_headline_len.py` 已改版（2026-09-14）**：`CAP_EN['signal']` 25 → 34；新增 `xpulse` 類別，先從 `<div class="x-pulse">` 抽 h4 當 xpulse、再把該段從內文移除避免被 col 重複計。改版後同一批標題全部被正確抓出。

**docs/index.html 的 `.desc` 也要量**（W37 Reagan 抓到）：寬 756px、一行約 66 字元，上限 3 行 = **≤ 198 字元**。W37 第一版 210 字元排成 4 行、末行 4%，壓到 194 字元後是 3 行 77%。`prepare-for-github.sh` 不覆寫既有 index 條目，所以同週重跑要手改 `docs/index.html`。

**流程不變、但順序要記牢**：腳本負責寫作當下擋掉明顯超標，**線上實測負責最後確認**——而且實測抓到不合格時，要先問「是標題寫壞，還是門檻寫錯」。W35、W36、W37 連三期都是門檻錯，三次都先改標題、只有 W36 跟 W37 回頭改了工具。
