...
## 2026-09-13（W37）— 篇幅控制要在写之前分配，不是写完再砍；`_shared/` 连续第二週不可达

### 教训 1：第一版 md 107KB / html 161KB，是「材料多」而不是「内容好」

W37 的产业新闻密度确实高（预测市场诉讼、巴西临时措施令、柬埔寨全国暂停、Moody's 首次评等四条主线同週），第一版写了 **59 张 card**，md 107KB、html 161KB——html 比 SKILL.md 的 85–115KB 上限超 40%。砍到 37 张 col-card（39 张含 §9）＋ 逐条收短 body 与 judge 之后，html 112.8KB、EN 121.4KB、md 70.7KB。

**根因**：写之前没有先分配篇幅。card 数与每张的 body／judge 字数是两个独立变数，我只控了「要写哪些事」、没控「每张给多少字」，结果每张 body 平均 290 字、judge 208 字（W36 约各 200／160），乘上 59 张就爆掉。

**修法（下週起照办）**：开写前先算配额——`目标 html 105KB ÷ 3 bytes ≈ 35,000 字 → 扣掉讯号区 5,000、§9 2,500、§10 与骨架 2,500 → 25,000 字 ÷ 目标卡数`。40 张卡就是每张 body + judge ≤ 620 字（含标题与来源行约 650）。超过就砍卡，不要两边都塞。
**砍卡的优先序**（本次实测有效）：跟 5 大讯号同一事件的展开卡 → 与 Reagan 主战场（PH／BR／VN）无关的实体赌场资本动作 → 同一市场的第二张宏观数据卡（合并成一张、把第二条的关键数字塞进第一张的 body 与 judge）→ 单独看有趣但无可操作动作的国际个案。
**合并的正确做法**：砍掉澳门 UBS 卡时，把「9 月前六日日均 −11.5%」「十一长假均价 −22%」「博彩税完成全年目标 71.5%」三个数字併进 DSEC 回佣卡的 body 与 judge，资讯没掉、少一张卡。砍柬埔寨 CCTC 卡时同理，把 18／9／195／72 四个数字併回讯号 05 的 body。

### 教训 2：`check_headline_len.py` 的 EN 上限要用「当量」而不是「字元」在心里换算

写 EN 标题时我按「col 50–72 字元」抓长度，但脚本算的是**当量**（ASCII 0.5、非 ASCII 1），所以 `€16.4m` 里的 `€` 算 1 不是 0.5，`GiG buys 80% of Evoke's 888Africa for up to €16.4m, paying €6m in cash.` 71 字元实际是 36.5 当量、刚好破 36 的门槛。signal 的上限更容易误判：cap 25 当量 = 50 字元单行、两行上限 50 当量 = **100 字元**，我写了 113 字元的 signal 02，排成三行。

**修法**：EN 标题写完先自算当量（把每个非 ASCII 字元多算 0.5），或者直接留安全边际——signal ≤ 95 字元、col ≤ 68 字元，再跑脚本。本次 6 个不合格标题全部落在 71–133 字元区间，都是「以为还有空间」造成的。

### 教训 3：`_shared/` 连续第二週不可达，防呆规则有效但根因没解

W36 的防呆写进 SOP 之后，W37 第一步就跑 `test -d /sessions/<id>/mnt/_shared` → UNREACHABLE，§0 直接写「trigger 来源不可达、待补」，收尾五行也标出来，没有再犯「推定为空」的错。**规则生效了。**

但根因还在：排程沙箱的 mount 清单只有 `daily-brief/`，`_shared/` 不在里面，所以 trigger monitor 的产出对 weekly 永远不可见。这不是 weekly 端能修的——要嘛把 `_shared/` 加进排程任务的挂载清单，要嘛让 trigger-monitor 改写到 `daily-brief/_shared/`（同一棵树、必然挂得到）。**建议走后者**，因为挂载清单是 app 端设定、改不了就一直卡着。已列为待办，请 Reagan 决定。

### 教训 4：hilite 卡挑代表来源时要先扫一遍已用过的 cta URL

`_design-notes §8.2` 允许 Editor's Pick 挑「代表性最强的一条 source 加 cta」，但没说要避开已经用过的。本次 §5 与 §8 两张 hilite 各自挑了讯号 04 与 §8 第二张卡已经在用的 URL，`§7.20` 的重复 URL 检查直接抓到 2 处。改法是从**被砍掉那些卡**的 URL 里挑（Acesite 的 IAG 链、Clase 案的 InGame 链），既没重复、又刚好是 hilite body 里真的提到的事件。
**加进收尾 SOP**：hilite 的 cta 决定之前，先跑一次 `grep -o 'class="cta"[^>]*href="[^"]*"'` 看清单，挑没出现过的。

### 本期数据留档

| 项目 | 数值 |
|---|---|
| card 总数 | 39（37 col-card + §9 两张）；含 7 张 Editor's Pick |
| 讯号 | 5（lead 1 + signal-card 4） |
| md / html CN / html EN | 70.7KB / 112.8KB / 121.4KB |
| 综合判断 per card | 1.13（红线 0.40） |
| pills / highlight / cta per card | 1.15 / 1.10 / 1.13 |
| 跨週标题重复率 | **0.0%**（W36 43 个标题 vs W37 44 个，完全相同 0 个） |
| de_ai_lint | 三档 0 命中（一轮修掉 1 处「你们没改」） |
| check_headline_len | CN 一次通过；EN 修 6 个后通过 |
| §9 X Pulse | 自跑成功，NA 5 条（4 条 HIGH）/ BR 5 条（1 条有料）/ VN 5 条，VN 併入 §5 |
| PH promo | 1/7 取到数字、与 W36 完全相同、0 家改 → 不写 §5 promo 卡 |
| §0 | trigger 来源不可达、待补 |
| git push | 403（沙箱 egress，与 W36 同）；commit c485817 已在本机 |

---

## 2026-09-14（W37 第二輪）— 錯的門檻會反向誤導寫作；§9 的卡從來沒被單獨量過

上線後依 `_design-notes §7.21` 複量，CN 與 EN 各 4 個孤行，`check_headline_len.py` 全部回報合格。細節與新真值表寫進 `_design-notes.md §7.24`，這裡只記教訓。

### 教訓 1：工具給錯的紅燈，跟給錯的綠燈一樣有害

W36 的結論是「工具給了錯誤的綠燈要修工具」。W37 踩到反面：`CAP_EN['signal']` 寫 25 當量（= 50 字元），實測是 65–70 字元，**低了一倍**。後果不是放行爛標題，是**把好標題擋掉**——EN 訊號標題我本來寫 102–133 字元（實測末行 45–61%、完全合格），被這個錯門檻判成三行，砍到 71–83 字元才變成末行 7–18% 的孤行。第二輪把四個標題還原成接近原稿的長度就全過了。

**下次遇到腳本擋標題，先問「是我寫太長，還是門檻寫錯」**，而不是直接砍。判斷方法很便宜：拿上一期的線上頁當量測台，把候選標題注入同一個元素量一次（`§7.21` 的 `__try` 做法），30 秒就知道門檻對不對。W35、W36、W37 連三期都是門檻錯。

### 教訓 2：class 相同不等於寬度相同——§9 的卡是 504px、不是 318px

§9 的兩張卡 class 是 `.card`，所以檢查器把它們當 col（318px、CN 21 字）量。但它們在 `.x-pulse` 兩欄格線裡、實寬 **504px**。結果 §9 巴西卡 37.5 當量被算成「兩行、末行 78%」，實際是兩行、末行 **9%**——整份最難看的一個孤行，從 W18 以來每一期都可能中，只是沒人量過。

**修法**：`check_headline_len.py` 新增 `xpulse` 類別，先抽 `.x-pulse` 的 h4、再把該段從內文移除避免重複計。
**通則**：量測台的分類要按**實際渲染寬度**分，不是按 class 名分。往後只要 canonical 新增任何非 `.grid-auto` 的卡片容器，就要在檢查器裡開一個對應類別。

### 教訓 3：`docs/index.html` 的 desc 也是排版元素，要量

Reagan 直接指出「index 頁的 W37 這邊就要少二個字元、也多了一行」。實測 desc 寬 756px、一行約 66 字元，**上限 3 行 = 198 字元**；W37 寫了 210 字元，排成 4 行、末行 4%（就是那「二個字元」）。壓到 194 字元後 3 行 77%。

過去只量 weekly 內頁的 h3 / h4，index 的 desc 一直靠感覺寫。已寫進 `_design-notes §7.24`。另注意 `prepare-for-github.sh` **不覆寫既有 index 條目**，同週重跑腳本 desc 不會更新，要手改 `docs/index.html`。

### 教訓 4：zsh 沒開 `interactive_comments`，給指令一律不要帶註解

第一次給 Reagan 的指令塊每行前面有 `# 1)` 這種註解，他的 zsh 沒開 `interactive_comments`，`#` 不被當註解、`)` 直接 parse error，而且**整段貼上的 buffer 會被一起丟掉**，所以五行指令一行都沒跑、連跑三次都同一個錯。改成純指令零註解後一次過。

**往後給本機指令塊：零 `#` 註解、零 `<<<` 這類會被 shell 解讀的符號，說明寫在指令塊外面。**

### 第二輪改動清單

| 檔 | 改了什麼 |
|---|---|
| `build_w37_html.py` | 訊號 04 補「與隨之而來的多級下調」37%→66%；訊號 05 補「並面臨法律追訴」37%→49%；§8 NFL 補「的根本」37%→50%；§9 巴西卡改寫 9%→47% |
| `build_w37_html_en.py` | 訊號 02 / 03 / 04 / 05 還原成 102–136 字元、末行 45–61% |
| `scripts/check_headline_len.py` | `CAP_EN['signal']` 25→34；新增 `xpulse` 類別 |
| `docs/index.html` | W37 desc 210→194 字元（4 行 4% → 3 行 77%） |
| `_design-notes.md` | 新增 §7.24 實測容量表，取代 §7.23 |

改完 CN 與 EN 都是「✓ 全部單行或兩行飽滿、無孤行」，`de_ai_lint` 三檔 0 命中，per-card 比例不變（judge 1.13 / pills 1.15 / hi 1.10 / cta 1.13）。
